...
/** A saved id is a preference, not proof that a trigger still exists or accepts focus. */
...
export function WorkspaceHome({ fixture, ticket, children, threadActive, onThread, onHome, scenarioControl, exceptionalState }: { fixture: WorkspaceFixture; ticket: SyntheticTicket; children: ReactNode; threadActive: boolean; onThread: (threadId: string) => void; onHome: () => void; scenarioControl: ReactNode; exceptionalState: ReactNode }) {
  ...
  // One consumable intent prevents search and return-focus requests from competing.
  const pendingFocus = useRef<"search" | "trigger" | null>(null);
  ...
  const search = useCallback(() => {
    ...
    setExpanded(false);
    pendingFocus.current = focusElement(searchRef.current) ? null : "search";
  }, [goHome]);
  useLayoutEffect(() => {
    if (showThread || pendingFocus.current === null) return;
    const request = pendingFocus.current;
    pendingFocus.current = null;
    if (request === "trigger" && focusElement(document.getElementById(triggerId.current))) return;
    // The search input survives an empty/filtered list, including the last removed favorite.
    if (!focusElement(searchRef.current)) focusElement(menuRef.current);
  });
  ...
  const select = (item: WorkbenchItem, trigger: HTMLButtonElement) => {
    ...
    setNotice("");
    if (item.ticket && item.threadId) openThread(item.threadId);
  ...
  const closeDetail = () => { pendingFocus.current = "trigger"; setSelected(null); };
  const detail = selected && <WorkItemDetail favorite={favoriteIds.has(selected.id)} item={selected} onClose={closeDetail} onThread={openThread} onToggleFavorite={() => toggleFavorite(selected.id)} />;
  ...
  return <div className="wb-shell" onKeyDown={(event) => { if (event.key === "Escape" && expanded) { event.stopPropagation(); closeSidebar(); } }}><WorkspaceSidebar expanded={expanded} favoriteCount={favoriteIds.size} favoritesOnly={favoritesOnly} homeActive={!showThread} items={items} mine={mine} onAgent={() => { goHome(); setExpanded(false); setNotice("Agent 尚未接入：没有模型调用、后台任务或工具执行。可在 Agent 协作空间查看固定合成提案。"); }} onClose={closeSidebar} onFavorites={() => { goHome(); setFavoritesOnly(true); setSpaceId(""); setMine(false); setQuery(""); if (expanded) closeSidebar(); }} onNavigate={(id, onlyMine) => { goHome(); setFavoritesOnly(false); setSpaceId(id); setMine(onlyMine); setQuery(""); if (expanded) closeSidebar(); }} onSearch={search} spaceId={spaceId} spaces={fixture.spaces} /><div className="wb-main"><header className="wb-topbar"><div><button aria-controls="wb-sidebar" aria-expanded={expanded} className="wb-menu" onClick={() => setExpanded(!expanded)} ref={menuRef} type="button">{expanded ? "收起导航" : "打开导航"}</button><span>工作空间 / {showThread ? "事项上下文" : "协作工作台"}</span></div><span className="wb-demo">公开合成演示</span></header>{showThread ? <><div className="wb-back"><button onClick={() => { goHome(); pendingFocus.current = "trigger"; }} type="button">← 返回工作台</button>{scenarioControl}</div>{children}</> : homePage}</div></div>;
}
